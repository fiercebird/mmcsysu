...
$this->widget('zii.widgets.grid.CGridView', array(
         ...
         'columns'=>array(
            ...
            array(
               ...
               'deleteConfirmation'=>"js:'确定要把该助理\"'+$(this).parent().parent().children(':eq(2)').children().html()+'\"放入回收站？'",
               'afterDelete'=>'function(link,success,data){ if(success) {var res=JSON.parse(data);if(res.resCode){var hint="<div class=\'flash-error mesFade\'>" + res.resMes + "</div>"; }else{var hint="<div class=\'flash-success mesFade\'>删除助理成功！</div>";}$("#hint").html(hint); $(".mesFade").animate({opacity: 1.0}, 2000).fadeOut(3000); }}',
               'buttons'=>array(
                  'view'=>array(      
                     'label'=>'',      
                     'imageUrl'=>'',
                     'url'=>'Yii::app()->controller->createUrl("view", array("id"=>$data->article_id, "cate"=>$data->category_id, "title"=>$data->title))',
                     'options'=>array('class'=>'icon-search', 'title'=>'查看助理' ),
                     ),  
                  'update'=>array(      
                     'label'=>'',      
                     'imageUrl'=>'',
                     'url'=>'Yii::app()->controller->createUrl("update", array("id"=>$data->article_id, "title"=>$data->title))',
                     'options'=>array('class'=>'icon-edit', 'title'=>'编辑助理' ),
                     ),  
                  'delete'=>array(      
                     'label'=>'',          
                     'imageUrl'=>'',
