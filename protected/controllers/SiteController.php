...
class SiteController extends Controller
{
	...
	public function actionIndex()
	{
		...
		// using the default layout 'protected/views/layouts/main.php'
	   	$articles=Article::model()->serviceInfo()->publishedandSetTop()->recently()->findAll();
                ...
                if(!isset($contactWay))
                   Yii::log( '首页不能获取联系人！','warning','db' . $this->action->id);
	...
	public function actionTechExplore()
        {
                 ...
                 * 
                 */
                //$dataProvider = Article::model()->findAll($criteria);
                $dataProvider=Article::model()->techExplode()->findAll();			   
	...
	public function actionSpecial()
	{
        ...
        public function actionClassroomDetail()
        {
				...
				$classDetail = Classroom::model()->findByAttributes(
					array(
						'className'=>$className,
						'bid'=>$bid
					));
                ...
                $this->render('classroomDetail',array('classDetail'=>$classDetail,'classroom'=>$classroom,'bid'=>$bid, 'className'=>$className));
        }
        ...
        public function actionServiceList()
        {
                ...
                if(isset($_GET['typeid'])) {
                    ...
                    $this->redirect(array('serviceList', 'typeid' => MisService::WEEK_SERVICE));
                }
                $this->render('serviceList',array('model' => $model,'typeid' => $typeid));
        }
        ...
        //优秀助理
        public function actionPerfectAssistant()
        {
               Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/slides.min.jquery.js',CClientScript::POS_HEAD);
               $this -> layout = 'column4';
               $assistants=Article::model()->perfectAssistants()->findAll();
               /*
               if(empty($assistants))
                       Yii::log('Article about assistant is null', 'warning', 'db.actionRule');
                * 
                */
               $this -> render('perfectAssistant',array('assistants'=>$assistants));               
        }
        
        //工作感想
        public function actionWorkFeeling()
        {
               Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/slides.min.jquery.js',CClientScript::POS_HEAD);
               $this -> layout = 'column4';
               $feelings=Article::model()->workFeelings()->findAll();
               /*
                if(empty($feelings))
                       Yii::log('Article about feeling is null', 'warning', 'db.actionRule');
                * 
                */
               $this -> render('workFeeling',array('feelings'=>$feelings));
        }
        ...
        public function actionActivityReport()
        {
               Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/slides.min.jquery.js',CClientScript::POS_HEAD);
               $this -> layout = 'column4';
               $reports=Article::model()->activityReports()->findAll();
               /*
                if(empty($reports))
                       Yii::log('Article about report is null', 'warning', 'db.actionRule');
                * 
                */
               $this -> render('activityReport',array('reports'=>$reports));
        }
